...
class RpWalletDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable
            ...
            ->with('deposit', function () use ($query) {
                return core()->currency((clone $query)->where('credit_type', 'D')->sum('total'));
            })
            ->setTransformer(new RpWalletTransformer(request()->get('start')));
    ...
    protected function getColumns()
    {
        return [
            ...
            ['data' => 'balance_before', 'name' => 'members_credit_log.credit_before', 'title' => 'Wallet (ก่อน)', 'orderable' => false, 'searchable' => false, 'className' => 'text-right text-nowrap'],
            ['data' => 'balance_after', 'name' => 'members_credit_log.credit_balance', 'title' => 'Wallet (หลัง)', 'orderable' => false, 'searchable' => false, 'className' => 'text-right text-nowrap'],
