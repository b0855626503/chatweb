...
class RpSetPointDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable
            ...
            ->with('deposit', function () use ($query) {
                return core()->currency((clone $query)->where('point_type', 'D')->sum('point_amount'));
            })
            ->setTransformer(new RpSetPointTransformer(request()->get('start')));
     ...
     * @return mixed
     */
    public function query(MemberPointLog $model)
    {
        ...
        return $model->newQuery()
            ...
            ->when($user, function ($query, $user) {
                $query->whereIn('members_pointlog.member_code', function ($q) use ($user) {
    ...
    {
        return [
            ['data' => 'code', 'name' => 'members_pointlog.code', 'title' => '#', 'orderable' => true, 'searchable' => false, 'className' => 'text-center text-nowrap'],
            ...
            ['data' => 'balance_before', 'name' => 'members_credit_log.credit_before', 'title' => 'Point (ก่อน)', 'orderable' => false, 'searchable' => false, 'className' => 'text-right text-nowrap'],
            ['data' => 'balance_after', 'name' => 'members_credit_log.credit_balance', 'title' => 'Point (หลัง)', 'orderable' => false, 'searchable' => false, 'className' => 'text-right text-nowrap'],
